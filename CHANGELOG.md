...
### Added
- **Annotations**: Introduced `@BlocMeta`, `@CubitMeta`, and `@EventMeta` annotations for defining BLoC components.
...
### Changed
- **Breaking**: Renamed `@BlocMeta` to `@BlocMeta`, `@CubitMeta` to `@CubitMeta`, and `@EventMeta` to `@EventMeta`.
